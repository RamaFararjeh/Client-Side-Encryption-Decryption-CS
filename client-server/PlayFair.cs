...
namespace client_server
{
    public partial class PlayFair : Form
    {
        ...
        private void btnSendMessage_Click(object sender, EventArgs e)
        {
            try
            {
                if (Client.frm_Instace_client._client.Connected)
                {
                    StreamWriter writer = new StreamWriter(Client.frm_Instace_client._client.GetStream());
                    writer.WriteLine("C1-" + txtCipher.Text+"-"+2+"-"+txtKeyWord.Text);
                    writer.Flush();
                    Client.frm_Instace_client.txt_Status_Instance.Text += CRLF + "Command Sent to Server: " + txtCipher.Text;
            ...
            {
                Client.frm_Instace_client.txt_Status_Instance.Text += CRLF + "Problem sending command to server...";
                Client.frm_Instace_client.txt_Status_Instance.Text += CRLF + ex.ToString();
            }
        }
        ...
        {                      
            ...
            for (int i = 0; i < 5; i++)
            {
                for (int j = 0; j < 5; j++)
                {
                    ...
                    //MessageBox.Show(playfairMatrix[i, j].ToString());
                    index++;
                }
            }
            ...
            //MessageBox.Show(playfairMatrix[1, 4].ToString()); //print letter in index[4,4]
            ...
            while (plain_text.Length !=0)
            {
                ...
                if(plain_text.Length == 1)
                {
                    ...
                    {
                        cipher_text += playfairMatrix[first_index.Item1, (first_index.Item2 + 1) %5];
                        cipher_text += playfairMatrix[sec_index.Item1, (sec_index.Item2 + 1) %5];
                    ...
                    {
                        cipher_text += playfairMatrix[(first_index.Item1 + 1) %5, first_index.Item2];
                        cipher_text += playfairMatrix[(sec_index.Item1 + 1) %5, sec_index.Item2];
        ...
        {
            new_cipher = txtCipher.Text;
           /* txtCipher.Text = "";
            txtKeyWord.Text = "";*/ // ibrahem 
            ...
            if (txtplaintext.Text == "")
            {
                ...
                //MessageBox.Show(playfairMatrix[1, 4].ToString());
                ...
                while (new_cipher.Length != 0)
                {
                    if (new_cipher[0] != new_cipher[1])
                    {
                        ...
                        else if (first_index.Item2 == sec_index.Item2) // to check if this the same col
                        {
                           ...
                           // MessageBox.Show(mod((first_index.Item1 - 1), 5).ToString());
                            // MessageBox.Show(sec_index.Item2.ToString());
